...
void update_borders(int *grid[], int prev, int this, int next, int size, int n, MPI_Status s)
{

	MPI_Request r1, r2, r3, r4;
	
	MPI_Isend(grid[1], SIZE, MPI_INT, prev, 10, MPI_COMM_WORLD, &r1);
	
	//MPI_Barrier(MPI_COMM_WORLD);
	MPI_Irecv(lower_ghost, SIZE, MPI_INT, this, 10 ,MPI_COMM_WORLD, &r2);
	

	MPI_Isend(grid[size],SIZE, MPI_INT, next, 11, MPI_COMM_WORLD, &r3);
	MPI_Irecv(upper_ghost, SIZE, MPI_INT, this, 11 ,MPI_COMM_WORLD, &r4); 
	
	
		
	/*MPI_Sendrecv(grid[1], SIZE, MPI_INT, posterior , 11,
            	 grid[size+1], SIZE, MPI_INT, anterior, 11,
                 MPI_COMM_WORLD, &s);*/

		
    /*MPI_Sendrecv(grid[size], SIZE, MPI_INT,  ,11,
                grid[0], SIZE, MPI_INT, , 11,
                MPI_COMM_WORLD, &s);*/
...
void update_ghosts(int *grid[], int size)
{
	for (int i = 0; i < SIZE; i++){
		grid[0][i] = upper_ghost[i];
		grid[size+1][i] = lower_ghost[i]; 
	}
...
void new_generation(int size, int total_sum, int id, int prev, int next, int n)
{
    int count = 0;
    ...
    for (int g = 0; g < GENERATIONS; g++) {
		update_borders(grid, prev, id, next, size, n, s);
		update_ghosts(grid, size);
		MPI_Barrier(MPI_COMM_WORLD);
...
int main(int argc, char *argv[])
{
    ...
    prev_proc = (proc_id+num_proc-1) % num_proc;
    ...
    /*********************** novas gerações ***********************/
    new_generation(ratio, total_sum, proc_id, prev_proc, next_proc, num_proc);
